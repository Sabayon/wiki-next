...
given we use graphical boot splash technology like [[En:Plymouth]] these days. 
...
{{Console| <pre class="clear">timedatectl set-time 2013-11-30 18:17:16</pre>}}
...
* Install NTP:
{{Console| <pre class="clear">equo i net-misc/ntp</pre>}}


* Enable service to start at boot and start it now:
{{Console| <pre class="clear">systemctl enable ntpd && systemctl start ntpd</pre>}}


* Enable time synchronization with NTP:
{{Console| <pre class="clear">timedatectl set-ntp 1</pre>}}
...
{{Console| <pre class="clear">localectl set-locale LANG=en_US.UTF-8</pre>}}


To set the virtual console keyboard map to US, enter:
{{Console| <pre class="clear">localectl set-keymap us it</pre>}} 
...
{{Console| <pre class="clear">localectl set-x11-keymap us apple mac_nodeadkeys numpad:pc</pre>}}
...
{{Console| <pre class="clear">systemd-analyze</pre>}}


To list the started unit files, sorted by the time each of them took to start up:
{{Console| <pre class="clear">systemd-analyze blame</pre>}}
...
{{Console| <pre class="clear">systemd-analyze critical-chain</pre>}}


You can also create a SVG file which describes your boot process graphically, similiar to Bootchart:
{{Console| <pre class="clear">systemd-analyze plot > plot.svg</pre>}}
...
To enable, do:
{{Console| <pre class="clear">systemctl enable systemd-readahead-collect systemd-readahead-replay</pre>}}
...
{{Console| <pre class="clear">nano -w /usr/lib/systemd/system/tmp.mount</pre>}}
...
{{Console| <pre class="clear">/etc/updatedb.conf</pre>}}
...
{{Console| <pre class="clear">nano -w /etc/fstab
